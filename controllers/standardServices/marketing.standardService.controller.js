...
import MarketingStandardService from '../../models/standardServices/marketing.standardService.model.js';
...
const addOneStandardService = asyncWrapper(async (req, res) => {
  ...
  if (
    req.user.role == adminRole.marketingStandardServiceCreator ||
    req.user.role == adminRole.admin
  ) {
    const { title, image, priceEgypt, priceSaudi } = req.body;
    const service = await MarketingStandardService.create({
      title,
      image,
      priceEgypt,
      priceSaudi,
...
const getAllStandardService = asyncWrapper(async (req, res) => {
  ...
  if (country == 'EG') {
    const services = await MarketingStandardService.find(filter, { __v: false, priceSaudi: false })
      .skip(skip)
      .limit(limit);

    const total = await MarketingStandardService.countDocuments(filter);
  ...
  const services = await MarketingStandardService.find(filter, { __v: false, priceEgypt: false })
    .skip(skip)
    .limit(limit);
...
const getAllStandardServiceAllPrices = asyncWrapper(async (req, res) => {
  ...
  if (
    ...
    const services = await MarketingStandardService.find(filter, { __v: false })
      .skip(skip)
      .limit(limit);
    const total = await MarketingStandardService.countDocuments(filter);
...
const updateStandardService = asyncWrapper(async (req, res) => {
  ...
  const services = await MarketingStandardService.findById(id);
  if (!services) {
...
const deleteOneStandardService = asyncWrapper(async (req, res) => {
  ...
  const result = await MarketingStandardService.deleteOne({ _id: id });
