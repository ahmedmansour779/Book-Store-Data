...
import ProgrammingStandardService from '../../models/standardServices/programming.standardService.model.js';
...
const addOneStandardService = asyncWrapper(async (req, res) => {
  ...
  if (
    req.user.role == adminRole.programmingStandardServiceCreator ||
    req.user.role == adminRole.admin
  ) {
    const { title, image, priceEgypt, priceSaudi } = req.body;
    const service = await ProgrammingStandardService.create({
      title,
      image,
      priceEgypt,
      priceSaudi,
...
const getAllStandardService = asyncWrapper(async (req, res) => {
  ...
  if (country == 'EG') {
    const services = await ProgrammingStandardService.find(filter, {
      __v: false,
      priceSaudi: false,
    })
      .skip(skip)
    ...
    const total = await ProgrammingStandardService.countDocuments(filter);
  ...
  const services = await ProgrammingStandardService.find(filter, { __v: false, priceEgypt: false })
    .skip(skip)
    .limit(limit);
...
const getAllStandardServiceAllPrices = asyncWrapper(async (req, res) => {
  ...
  if (
    ...
    const services = await ProgrammingStandardService.find(filter, { __v: false })
      .skip(skip)
      .limit(limit);
    const total = await ProgrammingStandardService.countDocuments(filter);
...
const updateStandardService = asyncWrapper(async (req, res) => {
  ...
  const services = await ProgrammingStandardService.findById(id);
  if (!services) {
...
const deleteOneStandardService = asyncWrapper(async (req, res) => {
  ...
  const result = await ProgrammingStandardService.deleteOne({ _id: id });
